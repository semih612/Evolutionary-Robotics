...
class EvalHillEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        lateral_cost = float(1.0 * y_position ** 2 + 0.1 * y_velocity ** 2)
        lane_violation = abs(y_position) > 1.5

        terminated = self._is_terminated(xyz_velocity) or lane_violation
        reward = (
            healthy_reward
            + 0.5 * x_position
            + 2.0 * x_velocity
    ...
    def _is_terminated(self, xyz_velocity: np.ndarray) -> bool:
        ...
        if np.linalg.norm(xyz_velocity) < 1e-2:
            self._stuck_count += 1
            if self._stuck_count > 2 / self.dt:
                return True
        else:
            self._stuck_count = 0
        return False
